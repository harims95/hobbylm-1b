...
def smoke():
    ...
    from hobbylm.multimodal import MoEVLM, IMAGE_TOKEN, IGNORE_INDEX
...
def audio_smoke():
    ...
    os.chdir("/root/moe-lab"); sys.path.insert(0, "/root/moe-lab")
    import tiktoken
    from hobbylm.audio import ClapAudio, CLAP_SR
    from hobbylm.multimodal import MoEVLM, AUDIO_TOKEN
    from hobbylm.generate import load_model
...
def train_audio_stage1(max_steps: int = 1200, micro: int = 16, lr: float = 1e-3, warmup: int = 80,
    ...
    from hobbylm.audio import ClapAudio
    from hobbylm.multimodal import MoEVLM
    from hobbylm.generate import load_model
    from hobbylm.vlm_audio_data import ClothoAudio, audio_collate
...
def caption_audio(audio_run: str = "500M_vlm_audio_stage1", n: int = 8, max_new: int = 32,
    ...
    from hobbylm.vlm_audio_data import ClothoAudio
...
def train_stage1(max_steps: int = 1500, micro: int = 32, lr: float = 1e-3, save_name: str = "500M_vlm_stage1"):
    ...
    out = f"/data/runs/{save_name}"
    cmd = ["torchrun", "--standalone", "--nproc_per_node=8", "training/vlm_stage1.py",
...
def train_stage2(max_steps: int = 1200, micro: int = 16, lr: float = 2e-5, save_name: str = "500M_vlm_stage2",
    ...
    os.chdir("/root/moe-lab")
    out = f"/data/runs/{save_name}"
    cmd = ["torchrun", "--standalone", "--nproc_per_node=8", "training/vlm_stage2.py",
...
def caption(stage1_run: str = "500M_vlm_stage1", n: int = 8, max_new: int = 32, prompt: str = "",
    ...
    import tiktoken
    from hobbylm.vision import SiglipVision
    from hobbylm.multimodal import MoEVLM, IMAGE_TOKEN
    from hobbylm.generate import load_model, GPT2_VALID, EOT
...
def ocr_smoke(n: int = 3, shards: int = 1):
    ...
    from hobbylm.stream_data import StreamOcr
    tok = tiktoken.get_encoding("gpt2")
...
def export_onnx(joint_run: str = "500M_vlm_joint12",
    ...
    from hobbylm.multimodal import Projector
...
def dump_embeds(joint_run: str = "500M_vlm_joint12",
    ...
    from PIL import Image
    from hobbylm.vision import SiglipVision
    from hobbylm.speech import WhisperSpeech
    from hobbylm.multimodal import Projector
    dev = torch.device("cuda")
...
def mtmd_vision(joint_run: str = "500M_vlm_joint12",
    ...
    subprocess.run([sys.executable, "export/to_gguf.py", "--ckpt", ckpt, "--out", llm], check=True)   # +chat template
    subprocess.run([sys.executable, "export/to_mmproj.py", "--ckpt", ckpt, "--out", mmproj], check=True)
...
def audio_mel_debug(joint_run: str = "500M_vlm_joint12",
    ...
    if not os.path.exists(llm):
        subprocess.run([sys.executable, "export/to_gguf.py", "--ckpt", ckpt, "--out", llm], check=True)
    if not os.path.exists(aproj):
        subprocess.run([sys.executable, "export/to_aproj.py", "--ckpt", ckpt, "--out", aproj], check=True)
...
def audio_embd_debug(joint_run: str = "500M_vlm_joint12",
    ...
    exp = f"/data/runs/{joint_run}/export"; ckpt = f"/data/runs/{joint_run}/model.pt"
    llm = f"{exp}/joint12-bailingmoe2.gguf"; aproj = f"{exp}/aproj-joint12.gguf"
    for src, out in [("export/to_gguf.py", llm), ("export/to_aproj.py", aproj)]:
    ...
    from hobbylm.speech import WhisperSpeech, WHISPER_SR
    from hobbylm.multimodal import Projector
...
def llamacpp_e2e(joint_run: str = "500M_vlm_joint12", prompt: str = "The capital of France is",
    ...
    subprocess.run([sys.executable, "export/to_gguf.py", "--ckpt", f"/data/runs/{joint_run}/model.pt", "--out", out],
                   check=True)
...
def newdata_smoke(n: int = 2):
    ...
    from hobbylm.stream_data import StreamSmolTalk, StreamMobileActions, StreamAria, aria_preprocess
...
def train_joint(max_steps: int = 1600, micro: int = 4, lr: float = 2e-5, save_name: str = "500M_vlm_joint",
    ...
    os.chdir("/root/moe-lab")
    out = f"/data/runs/{save_name}"
    cmd = ["torchrun", "--standalone", "--nproc_per_node=8", "training/vlm_joint.py",
    ...
    if stream_smol or stream_mobile or with_aria:
        ...
        if with_aria:
            ...
            if jp and not os.path.exists(aria_jsonl):
                from hobbylm.stream_data import aria_preprocess
...
def unified(stage2_run: str = "500M_vlm_stage2", audio_run: str = "500M_vlm_audio_stage1",
    ...
    import tiktoken
    from hobbylm.vision import SiglipVision
    from hobbylm.video import SiglipVideo
    from hobbylm.audio import ClapAudio
    from hobbylm.multimodal import MoEVLM, IMAGE_TOKEN, VIDEO_TOKEN, AUDIO_TOKEN
    from hobbylm.generate import load_model, GPT2_VALID, EOT
    from hobbylm.vlm_audio_data import ClothoAudio
...
def unified5(joint_run: str = "500M_vlm_joint5", n: int = 4, max_new: int = 48):
    ...
    from hobbylm.audio import ClapAudio
    from hobbylm.speech import WhisperSpeech
    from hobbylm.multimodal import MoEVLM, IMAGE_TOKEN, VIDEO_TOKEN, AUDIO_TOKEN, SPEECH_TOKEN
    from hobbylm.generate import load_model, GPT2_VALID, EOT
    from hobbylm.vlm_audio_data import ClothoAudio
    from hobbylm.vlm_va_data import VoiceAssistantQA
...
def image_voice(joint_run: str = "500M_vlm_joint5", n: int = 4, max_new: int = 48):
    ...
    from hobbylm.vision import SiglipVision
    from hobbylm.speech import WhisperSpeech, WHISPER_SR
    from hobbylm.multimodal import MoEVLM, IMAGE_TOKEN, SPEECH_TOKEN
    from hobbylm.generate import load_model, GPT2_VALID, EOT
...
def speech_smoke():
    ...
    import tiktoken
    from hobbylm.speech import WhisperSpeech, WHISPER_SR
    from hobbylm.multimodal import MoEVLM, SPEECH_TOKEN
    from hobbylm.generate import load_model
...
def train_speech_stage1(max_steps: int = 1500, micro: int = 12, lr: float = 1e-3, warmup: int = 100,
    ...
    from hobbylm.multimodal import MoEVLM
    from hobbylm.generate import load_model
    from hobbylm.vlm_speech_data import LibriSpeechASR, speech_collate
...
def caption_speech(speech_run: str = "500M_vlm_speech_stage1", n: int = 8, max_new: int = 48,
    ...
    from hobbylm.multimodal import MoEVLM, SPEECH_TOKEN
    from hobbylm.generate import load_model, GPT2_VALID, EOT
    from hobbylm.vlm_speech_data import LibriSpeechASR
...
def train_speech_sft(max_steps: int = 1200, micro: int = 8, lr: float = 2e-5, proj_lr: float = 1e-4,
    ...
    from hobbylm.multimodal import MoEVLM
    from hobbylm.generate import load_model
    from hobbylm.vlm_va_data import VoiceAssistantQA, va_collate
...
def vlm_eval_run(stage2_run: str = "500M_vlm_stage2", joint_run: str = "", tasks: str = "pope",
    ...
    import os, sys, json
    os.chdir("/root/moe-lab"); sys.path.insert(0, "/root/moe-lab"); sys.path.insert(0, "/root/moe-lab/eval")
    import torch
    ...
    # @register_model registry. The wrapper lives in eval/, which we put on sys.path above so the
    # bare `vlm_eval_harness` module name resolves (it in turn imports the `hobbylm` package).
...
def prep_speech_tool(n: int = 14000, val: int = 1000, repo: str = "nvidia/Nemotron-Agentic-v1", max_q: int = 180):
    ...
    from huggingface_hub import hf_hub_download
    from transformers import VitsModel, AutoTokenizer
    from hobbylm.speech_tool_data import extract_speech_tool, wav_to_b64
...
def agent_demo(joint_run: str = "500M_vlm_joint8", query: str = "What's the weather in Tokyo right now?",
    ...
    from hobbylm.multimodal import MoEVLM
    from hobbylm.generate import load_model
    from hobbylm.agent import agent_tool_call, agent_summarize
