...
from hobbylm.multimodal import MoEVLM, IMAGE_TOKEN, VIDEO_TOKEN
...
def main():
    ...
    tool_it = None
    if use_tools:
        from hobbylm.tool_data import ToolCallSFT, tool_collate_weighted
        tool_it = mk(ToolCallSFT(args.tools, max_len=2048, weighted=True), tool_collate_weighted)
    ttraj_it = None
    if use_ttraj:
        from hobbylm.tool_data import TrajectorySFT, tool_collate as _tc
        ttraj_it = mk(TrajectorySFT(args.text_traj, max_len=2048), _tc)
    stool_it = None
    if use_stool:
        from hobbylm.speech_tool_data import SpeechToolSFT, speech_tool_collate
    ...
    if use_sagent:
        from hobbylm.stream_data import StreamAgentic, stream_collate_text
        sagent_it = mk_stream(StreamAgentic(rank=rank, world=world), stream_collate_text, workers=2)
    if use_svlm:
        from hobbylm.stream_data import StreamVLM, stream_collate_vlm
        svlm_it = mk_stream(StreamVLM(rank=rank, world=world), stream_collate_vlm, workers=4)
    socr_it = None
    if use_socr:
        from hobbylm.stream_data import StreamOcr, stream_collate_vlm
    ...
    if use_smol:
        from hobbylm.stream_data import StreamSmolTalk, stream_collate_text
        smol_it = mk_stream(StreamSmolTalk(rank=rank, world=world), stream_collate_text, workers=2)
    if use_mobile:
        from hobbylm.stream_data import StreamMobileActions, stream_collate_text
        mobile_it = mk_stream(StreamMobileActions(rank=rank, world=world), stream_collate_text, workers=2)
    if use_aria:
        from hobbylm.stream_data import StreamAria, stream_collate_vlm
