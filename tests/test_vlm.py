...
from hobbylm.config import get_config
from hobbylm.model import MoETransformer
from hobbylm.multimodal import MoEVLM, IMAGE_TOKEN, AUDIO_TOKEN, IGNORE_INDEX
...
def test_sft_masking():
    ...
    from hobbylm.vlm_data import LlavaSFT, ENC, EOT
    d = tempfile.mkdtemp()
...
def test_video_splice():
    ...
    from hobbylm.multimodal import VIDEO_TOKEN
    VIS, NV = 1152, 20
    vlm = MoEVLM(MoETransformer(tiny_cfg()), vision_dim=VIS)
